...
masstools::setwd_project()
dir.create(
  "data_analysis/cytokine_affect_microbiome/IS/cytokine_stool_microbiome",
  recursive = TRUE
)
setwd("data_analysis/cytokine_affect_microbiome/IS/cytokine_stool_microbiome")
...
load(
  here::here(
    "data_analysis/mediation_analysis/Previous_Version/sample_wise_IS/stool_microbiome_vs_cytokine/mediation_result"
  )
)
idx1 <-
  match(unique(mediation_result$treat),
        stool_microbiome_variable_info$variable_id)
idx2 <-
  match(unique(mediation_result$mediator),
