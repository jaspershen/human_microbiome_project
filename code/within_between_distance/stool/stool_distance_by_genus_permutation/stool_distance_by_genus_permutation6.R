...
family_info <-
  readxl::read_xlsx("data/from_xin/Related People In iPOP_from.sophia.xlsx",
                    col_names = FALSE)
dir.create("data_analysis/combine_microbiome/distance/stool/permutation")
setwd("data_analysis/combine_microbiome/distance/stool/permutation")
...
load(
  here::here(
    "data_analysis/combine_microbiome/distance/stool/stool_braydist_by_genus"
  )
)
...
genus_list <- unique(stool_braydist_by_genus$genus)
length(genus_list)

for (j in 101:120) {
  cat(j)
  x <- stool_braydist_by_genus %>%
