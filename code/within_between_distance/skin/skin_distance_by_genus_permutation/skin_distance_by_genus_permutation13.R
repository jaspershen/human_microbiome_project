...
allGenus = as.vector(tax_table(physeq_skin)[, "Genus"])
...
load(
  here::here(
    "data_analysis/combine_microbiome/distance/skin/skin_braydist_by_genus"
  )
)
...
genus_list <- unique(skin_braydist_by_genus$genus)
length(genus_list)

for (j in 361:length(genus_list)) {
  cat(j)
  x <- skin_braydist_by_genus %>%
