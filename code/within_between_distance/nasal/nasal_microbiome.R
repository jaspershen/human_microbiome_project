...
dir.create("data_analysis/nasal_microbiome/")
dir.create("data_analysis/nasal_microbiome/cladogram/")
setwd("data_analysis/nasal_microbiome/cladogram/")

load("bc_dist_by_genus_NS.rda")
bc_dist_nasal = bc_dist_by_genus
rm(list = "bc_dist_by_genus")
dim(bc_dist_nasal)
unique(bc_dist_nasal$genus)
...
expression_data = physeqGenus_NS@otu_table@.Data %>%
  t() %>%
  as.data.frame()

variable_info = as.data.frame(physeqGenus_NS@tax_table@.Data)

sample_info = get_variable(physeq = physeqGenus_NS)

####remove some variables
###change to relative counts
##only remain taxs with median > 1e-5
# physeqGenus_NS2 = filter_taxa(physeq = physeqGenus_NS, flist = function(x) median(x) > 1e-5, prune =  TRUE)
##remove some variables which genus are not in temp_data
# physeqGenus_NS2 = filter_taxa(physeq = physeqGenus_NS, 
#                               flist = function(x) median(x) > 1e-5, prune =  TRUE)
#                               
# physeqGenus_NS2 =
#   subset_taxa(physeq = physeqGenus_NS, Genus %in% grep("Unclassified",temp_data$Genus, value = TRUE, invert = TRUE))
...
# ##
# physeqGenus_NS2 =
#   subset_taxa(physeq = physeqGenus_NS2, !Genus %in% "Unclassified_Actinobacteria")
...
final_p =
  p3 +
  geom_hilight(
    node = p3$data$node[which(p3$data$label == "c__Bacilli")],
    fill = ggsci::pal_lancet()(n = 9)[1],
    alpha = .4
  ) +
  geom_hilight(
    node = p3$data$node[which(p3$data$label == "c__Actinobacteria")],
    fill = ggsci::pal_lancet()(n = 9)[3],
    alpha = .4
  ) +
  geom_hilight(
    node = p3$data$node[which(p3$data$label == "c__Alphaproteobacteria")],
