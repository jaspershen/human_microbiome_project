...
setwd("data_analysis/combine_microbiome/distance/nasal/permutation")
...
genus_list <- unique(nasal_braydist_by_genus$genus)

for (j in 251:length(genus_list)) {
  cat(j)
  x <- nasal_braydist_by_genus %>%
