...
allGenus = as.vector(tax_table(physeq_nasal)[, "Genus"])
...
#       subdat = prune_taxa(allGenus == temp_genus, physeq_nasal)
...
#   dplyr::bind_rows()
#
# save(nasal_braydist_by_genus,
#      file = "nasal_braydist_by_genus",
#      compress = "xz")

load(
  here::here(
    "data_analysis/combine_microbiome/distance/nasal/nasal_braydist_by_genus"
  )
)
...
remain_genus <-
  remain_genus[!remain_genus %in% remove_name]
...
#     cat(idx, " ")
#     temp_genus = unique(nasal_braydist_by_genus$genus)[idx]
#     temp_data_between =
#       nasal_braydist_by_genus %>%
#       dplyr::filter(genus == unique(nasal_braydist_by_genus$genus)[idx]) %>%
#       dplyr::filter(dist != 1) %>%
...
plot =
  ...
  ggrepel::geom_text_repel(aes(
    ...
    label = ifelse(abs(cor_within) > 0.3 &
                     p_adjust_within_cor < 0.05, genus, NA)
  ))
plot
...
plot =
  ...
  ggrepel::geom_text_repel(aes(
    ...
    label = ifelse(abs(cor_between) > 0.3 &
                     p_adjust_between_cor < 0.05, genus, NA)
  ))
plot
# ggsave(plot, filename = "between_cor_p.pdf", width = 9, height = 7)


# ######after run the permutation test, then use this to calculate the p values for fc1 and fc2
...
#                  outlier.shape = NA,
#                  show.legend = FALSE) +
#     # geom_jitter(aes(type, dist, color = type),
#     #             alpha = 0.8,
#     #             show.legend = FALSE) +
#     # geom_hex(aes(type, dist, color = type),
