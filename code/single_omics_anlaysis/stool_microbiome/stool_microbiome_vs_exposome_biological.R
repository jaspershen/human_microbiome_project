...
#######work directory
setwd(masstools::get_project_wd())
setwd("data_analysis/stool_microbiome/stool_microbiome_exposome_biological")

#####match data
{
  sort(stool_microbiome_sample_info$Date)
  ...
  diff <- 
    lapply(stool_microbiome_sample_info$Date %>% as.Date(), function(x){
      x <- abs(x - as.Date(exposome_biological_sample_info$date.start))
      as.Date(exposome_biological_sample_info$date.start)[which(x >=0 & x <= 3)]
    }) 
  
  names(diff) <- stool_microbiome_sample_info$Date %>% as.character()
  ...
  diff <- 
    purrr::map2(diff, names(diff), .f = function(x,y){
      ...
      colnames(z) <- c("stool_microbiome", "exposome_biological")
      z
    }) 
  ...
  diff <- 
    ...
    mutate(exposome_biological = as.character(exposome_biological), 
           stool_microbiome = as.character(stool_microbiome)) %>% 
  ...
  diff$stool_microbiome.date <- as.Date(diff$stool_microbiome.date)
  ...
  temp$class[temp$class == "stool_microbiome"] = "Stool microbiome"
  diff$exposome_biological.class[diff$exposome_biological.class == "exposome_biological"] = "Exposome"
  diff$stool_microbiome.class[diff$stool_microbiome.class == "stool_microbiome"] = "Stool microbiome"
...
plot <-
  ...
  geom_segment(
    aes(
      ...
      xend = stool_microbiome.date,
      y = exposome_biological.class,
      yend = stool_microbiome.class
    ),
    color = "black",
    data = diff,
    show.legend = FALSE
...
# ggsave(plot, filename = "exposome_biological_stool_microbiome_match.pdf", width = 10, height = 7)
...
# stool_microbiome_sample_info <-
#   stool_microbiome_sample_info %>%
#   dplyr::filter(Date %in% unique(diff$stool_microbiome.date))
# 
# stool_microbiome_sample_info
# 
# stool_microbiome_expression_data <-
#   stool_microbiome_expression_data %>%
#   dplyr::select(one_of(stool_microbiome_sample_info$sample_id))
# 
# stool_microbiome_sample_info <-
#   stool_microbiome_sample_info %>%
#   dplyr::left_join(diff[,c("stool_microbiome.date", "stool_microbiome.group")] %>% dplyr::distinct(stool_microbiome.date, stool_microbiome.group),
#                    by = c("Date" = "stool_microbiome.date")) %>%
#   dplyr::rename(group = stool_microbiome.group)
# 
...
# 
# ###sort date
# stool_microbiome_sample_info$sample_id == colnames(stool_microbiome_expression_data)
...
stool_microbiome_sample_info$sample_id == colnames(stool_microbiome_expression_data)
colnames(stool_microbiome_expression_data) <- as.character(stool_microbiome_sample_info$date)
colnames(exposome_biological_expression_data) <- colnames(stool_microbiome_expression_data)
...
# dim(stool_microbiome_expression_data)
# 
# ###correct fiber for stool_microbiome
# stool_microbiome_expression_data1 <-
#   purrr::map(as.data.frame(t(stool_microbiome_expression_data)), .f = function(x){
#     temp_data <-
...
# 
# colnames(temp_data) <-
#   colnames(stool_microbiome_expression_data1) <-
#   colnames(stool_microbiome_expression_data)
# 
# 
# ###calculate correlation between stool_microbiome and exposome
...
# save(stool_microbiome_expression_data1, file = "stool_microbiome_expression_data1")
# 
# cor_value <-
#   cor(x = t(as.matrix(exposome_biological_expression_data)),
#       y = t(as.matrix(stool_microbiome_expression_data1)),
#       method = "spearman")
# 
# cor_value <-
#   cor_value %>%
...
#     value2 <- as.numeric(stool_microbiome_expression_data1[x[2],])
...
cor_value1 <-
  ...
  dplyr::left_join(stool_microbiome_variable_info, by = c("to" = "variable_id")) %>%
  dplyr::mutate(stool_microbiome_id = to) %>%
  dplyr::filter(!is.na(stool_microbiome_id))
...
for(idx in 1:nrow(cor_value1)) {
  ...
  temp_data <-
    data.frame(
      date = as.character(stool_microbiome_sample_info$date),
      exp = as.numeric(exposome_biological_expression_data[cor_value1$from[idx], ]),
      pro = as.numeric(stool_microbiome_expression_data1[cor_value1$to[idx], ]),
      stringsAsFactors = FALSE
    )
  plot <-
    temp_data %>%
    ...
    labs(
      ...
      y = paste("Stool microbiome: " , cor_value1$stool_microbiome_id[idx]),
      sep = ""
    ) +
    theme_bw() +
    theme(
...
###network for all the exposome and stool_microbiome
edge_data <-  
  cor_value1 %>% 
...
node_data %>% 
  dplyr::left_join(stool_microbiome_variable_info)
...
ggsave(
  plot1,
  filename = "exposome_biological_stool_microbiome_correlation_network.pdf",
  width = 8.5,
  height = 7,
  bg = "transparent"
)
