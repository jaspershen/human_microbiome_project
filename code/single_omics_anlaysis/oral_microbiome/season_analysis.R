...
# physeqGenus_OR
# 
# expression_data = phyloseq::otu_table(object = physeqGenus_OR)
# expression_data = 
#   expression_data@.Data %>% 
#   t() %>% 
#   as.data.frame()
# 
# sample_info = phyloseq::sample_data(object = physeqGenus_OR)
# sample_info = sample_info@.Data %>% 
#   do.call(cbind, .) %>% 
#   as.data.frame()
# 
# colnames(sample_info) = colnames(phyloseq::sample_data(object = physeqGenus_OR))
# 
# variable_info = phyloseq::tax_table(object = physeqGenus_OR)
...
# 
# colnames(oral_microbiome_expression_data) == oral_microbiome_sample_info$RandomID
# colnames(oral_microbiome_expression_data) = oral_microbiome_sample_info$SampleID
# rownames(oral_microbiome_expression_data) == rownames(oral_microbiome_variable_info)
# 
...
# sample_info$RandomID == oral_microbiome_sample_info$RandomID
# oral_microbiome_sample_info = sample_info
...
#   cat(idx, " ")
#   x = as.numeric(oral_microbiome_expression_data[idx, ])
#   x = (x - mean(x))/sd(x)
#   temp_data =
#     data.frame(days = oral_microbiome_sample_info$days,
#                weeks = oral_microbiome_sample_info$weeks,
#                months = oral_microbiome_sample_info$months,
#                x = x,
#                subject_id = oral_microbiome_sample_info$subject_id,
#                bmi = oral_microbiome_sample_info$BMI,
#                SSPG = oral_microbiome_sample_info$SSPG,
#                iris = oral_microbiome_sample_info$iris) %>%
...
#            title = paste(oral_microbiome_variable_info$Genus[idx],"/p value:", round(p_value, 4))) +
...
#       filename = file.path(
#         "smooth_plot",
#         paste(oral_microbiome_variable_info$Genus[idx], "pdf", sep = ".")
#       ),
#       width = 9,
#       height = 7
#     )
...
# }
# 
# names(p_value_all) = oral_microbiome_variable_info$variable_id[remain_idx]
# coeffs_all =
# coeffs_all %>%
# as.data.frame()
# 
# rownames(coeffs_all) = oral_microbiome_variable_info$variable_id[remain_idx]
# 
