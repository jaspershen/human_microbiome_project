...
# physeqGenus_NS
#
# expression_data = phyloseq::otu_table(object = physeqGenus_NS)
# expression_data =
#   expression_data@.Data %>%
#   t() %>%
#   as.data.frame()
#
# sample_info = phyloseq::sample_data(object = physeqGenus_NS)
# sample_info = sample_info@.Data %>%
#   do.call(cbind, .) %>%
#   as.data.frame()
#
# colnames(sample_info) = colnames(phyloseq::sample_data(object = physeqGenus_NS))
#
# variable_info = phyloseq::tax_table(object = physeqGenus_NS)
...
#
# colnames(ns_microbiome_expression_data) == ns_microbiome_sample_info$RandomID
# colnames(ns_microbiome_expression_data) = ns_microbiome_sample_info$SampleID
# rownames(ns_microbiome_expression_data) == rownames(ns_microbiome_variable_info)
#
...
#
# ###ns microbiome
# load("data_analysis/ns_microbiome/data_preparation/sample_info")
#
# dim(sample_info)
# dim(ns_microbiome_sample_info)
# sample_info$RandomID == ns_microbiome_sample_info$RandomID
# sample_info$sample_id == ns_microbiome_sample_info$SampleID
#
# sample_info =
#   sample_info[match(ns_microbiome_sample_info$SampleID, sample_info$sample_id),]
#
# sum(sample_info$sample_id == ns_microbiome_sample_info$SampleID)
#
# ns_microbiome_sample_info = sample_info
...
load("data_analysis/ns_microbiome/data_preparation/sample_info")
load("data_analysis/ns_microbiome/data_preparation/expression_data")
load("data_analysis/ns_microbiome/data_preparation/variable_info")

ns_microbiome_sample_info = sample_info
ns_microbiome_variable_info = variable_info
ns_microbiome_expression_data = expression_data

dim(ns_microbiome_variable_info)
...
#######work directory
setwd(masstools::get_project_wd())
setwd("data_analysis/ns_microbiome/season_analysis")
...
#   cat(idx, " ")
#   x = as.numeric(ns_microbiome_expression_data[idx, ])
#   x = (x - mean(x))/sd(x)
#   temp_data =
#     data.frame(days = ns_microbiome_sample_info$days,
#                weeks = ns_microbiome_sample_info$weeks,
#                months = ns_microbiome_sample_info$months,
#                x = x,
#                subject_id = ns_microbiome_sample_info$subject_id,
#                bmi = ns_microbiome_sample_info$BMI,
#                SSPG = ns_microbiome_sample_info$SSPG,
#                iris = ns_microbiome_sample_info$iris) %>%
...
#            title = paste(ns_microbiome_variable_info$Genus[idx],"/p value:", round(p_value, 4))) +
...
#       filename = file.path(
#         "smooth_plot",
#         paste(ns_microbiome_variable_info$Genus[idx], "pdf", sep = ".")
#       ),
#       width = 9,
#       height = 7
#     )
...
# }
#
# names(p_value_all) = ns_microbiome_variable_info$variable_id[remain_idx]
# coeffs_all =
# coeffs_all %>%
# as.data.frame()
#
# rownames(coeffs_all) = ns_microbiome_variable_info$variable_id[remain_idx]
#
