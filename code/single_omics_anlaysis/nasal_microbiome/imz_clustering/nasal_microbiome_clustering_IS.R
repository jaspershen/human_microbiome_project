...
#######work directory
setwd(masstools::get_project_wd())
dir.create("data_analysis/nasal_microbiome/clustering/Imz_phylum_IS",
           recursive = TRUE)
setwd("data_analysis/nasal_microbiome/clustering/Imz_phylum_IS")

head(nasal_microbiome_sample_info)
...
infection_idx <-
  which(nasal_microbiome_sample_info$CL4 == "Imz")
...
all_index <-
  ...
  purrr::map(function(i) {
    temp_info <-
      nasal_microbiome_sample_info[i,]
    idx <-
      which(nasal_microbiome_sample_info$subject_id == temp_info$subject_id)
    if (length(idx) < 5) {
      return(
        list(
    ...
    temp_sample_info <- nasal_microbiome_sample_info[idx,]
    day_diff <-
      as.numeric(temp_sample_info$Date - nasal_microbiome_sample_info[i,]$Date)
...
new_expression_data <-
  ...
  purrr::map(function(idx) {
    negtaive_h <-
      nasal_microbiome_expression_data[, idx$negtaive_h_id, drop = FALSE] %>%
      apply(1, mean)
    
    ee <-
      nasal_microbiome_expression_data[, idx$ee_id, drop = FALSE] %>%
      apply(1, mean)
      # `-`(negtaive_h)
    
    el <-
      nasal_microbiome_expression_data[, idx$el_id, drop = FALSE] %>%
      apply(1, mean)
      # `-`(negtaive_h)
    
    re <-
      nasal_microbiome_expression_data[, idx$re_id, drop = FALSE] %>%
      apply(1, mean) 
      # `-`(negtaive_h)
    
    positive_h <-
      nasal_microbiome_expression_data[, idx$positive_h_id, drop = FALSE] %>%
      apply(1, mean) 
      # `-`(negtaive_h)
...
#     dplyr::left_join(nasal_microbiome_variable_info, by = "variable_id")
#   
#   openxlsx::write.xlsx(
#     x = cluster_data,
#     file = file.path(
