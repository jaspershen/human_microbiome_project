...
{
  load(here::here(
    "data_analysis/lipidome/data_preparation/expression_data"
  ))
  load(here::here("data_analysis/lipidome/data_preparation/sample_info"))
  load(here::here("data_analysis/lipidome/data_preparation/variable_info"))
  }

lipidome_expression_data = expression_data
lipidome_sample_info = sample_info
lipidome_variable_info = variable_info
...
#   log(lipidome_expression_data + 1, 2)

lipidome_sample_info <-
  lipidome_sample_info %>%
  dplyr::mutate(Date = date)

lipidome_sample_info$Date =
  as.Date(lipidome_sample_info$Date, "%m/%d/%y")
...
#######work directory
setwd(masstools::get_project_wd())
dir.create("data_analysis/lipidome/Imz_clustering_IR",
           recursive = TRUE)
setwd("data_analysis/lipidome/Imz_clustering_IR")
...
infection_idx <-
  which(lipidome_sample_info$CL4 == "Imz")
...
all_index <-
  ...
  purrr::map(function(i) {
    temp_info <-
      lipidome_sample_info[i,]
    idx <-
      which(lipidome_sample_info$subject_id == temp_info$subject_id)
    if (length(idx) < 5) {
      return(
        list(
    ...
    temp_sample_info <- lipidome_sample_info[idx,]
    day_diff <-
      as.numeric(temp_sample_info$Date - lipidome_sample_info[i,]$Date)
...
new_expression_data <-
  ...
  purrr::map(function(idx) {
    negtaive_h <-
      lipidome_expression_data[, idx$negtaive_h_id, drop = FALSE] %>%
      apply(1, mean)
    
    ee <-
      lipidome_expression_data[, idx$ee_id, drop = FALSE] %>%
      apply(1, mean)
    # `-`(negtaive_h)
    
    el <-
      lipidome_expression_data[, idx$el_id, drop = FALSE] %>%
      apply(1, mean)
    # `-`(negtaive_h)
    
    re <-
      lipidome_expression_data[, idx$re_id, drop = FALSE] %>%
      apply(1, mean)
    # `-`(negtaive_h)
    
    positive_h <-
      lipidome_expression_data[, idx$positive_h_id, drop = FALSE] %>%
      apply(1, mean)
    # `-`(negtaive_h)
...
#     dplyr::left_join(lipidome_variable_info %>% dplyr::select(-membership),
#                      by = "variable_id")
#   
#   openxlsx::write.xlsx(
#     x = cluster_data,
