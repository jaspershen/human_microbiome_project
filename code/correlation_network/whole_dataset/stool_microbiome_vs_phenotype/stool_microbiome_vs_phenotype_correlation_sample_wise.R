...
######work directory
setwd(masstools::get_project_wd())
dir.create("data_analysis/correlation_network/whole_data_set/stool_microbiome_vs_phenotype_sample_wise")
setwd("data_analysis/correlation_network/whole_data_set/stool_microbiome_vs_phenotype_sample_wise")
...
###finally, for stool microbiome, 108 genus, for phenotype, 52 variables
...
stool_microbiome_sample_info$subject_id == phenotype_sample_info$subject_id
...
# stool_microbiome_phenotype_lm_adjusted_cor <-
#   lm_adjusted_cor(
...
#   )
# 
# stool_microbiome_phenotype_lm_adjusted_cor_spearman = stool_microbiome_phenotype_lm_adjusted_cor[[1]]
# 
# save(
#   stool_microbiome_phenotype_lm_adjusted_cor_spearman,
#   file = "stool_microbiome_phenotype_lm_adjusted_cor_spearman",
#   compress = "xz"
# )

load("stool_microbiome_phenotype_lm_adjusted_cor_spearman")

###here we use the lm_adjusted_cor
sum(stool_microbiome_phenotype_lm_adjusted_cor_spearman$p_adjust < 0.2)
...
writeDataTable(wb, sheet = 2, x = edge_data,
               colNames = TRUE, rowNames = FALSE)
saveWorkbook(wb = wb, file = "stool_microbiome_vs_phenotype.xlsx", overwrite = TRUE)
...
plot <-
  ...
  scale_color_manual(values = c(omics_color[c("Stool microbiome")], 
                                phenotype_color)) +
...
ggsave(plot,
       filename = "stool_microbiome_phenotype_cor_plot_example.pdf",
       width = 8.5,
       height = 7)
