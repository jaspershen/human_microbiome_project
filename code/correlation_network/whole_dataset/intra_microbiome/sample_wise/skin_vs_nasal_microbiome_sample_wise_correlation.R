...
load("skin_microbiome_nasal_microbiome_lm_adjusted_cor_spearman")

###here we use the lm_adjusted_cor
sum(skin_microbiome_nasal_microbiome_lm_adjusted_cor_spearman$p_adjust < 0.2)
...
writeDataTable(wb, sheet = 2, x = edge_data,
               colNames = TRUE, rowNames = FALSE)
saveWorkbook(wb = wb, file = "skin_microbiome_vs_nasal_microbiome.xlsx", overwrite = TRUE)
...
plot <-
  ...
  scale_color_manual(values = omics_color[c("nasal_microbiome", "skin microbiome")]) +
...
ggsave(plot,
       filename = "skin_microbiome_nasal_microbiome_cor_plot_example.pdf",
       width = 8.5,
       height = 7)
