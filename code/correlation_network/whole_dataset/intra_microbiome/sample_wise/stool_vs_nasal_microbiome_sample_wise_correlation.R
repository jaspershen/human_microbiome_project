...
load("stool_microbiome_nasal_microbiome_lm_adjusted_cor_spearman")

###here we use the lm_adjusted_cor
sum(stool_microbiome_nasal_microbiome_lm_adjusted_cor_spearman$p_adjust < 0.2)
...
node_data =
  ...
  dplyr::left_join(nasal_microbiome_variable_info[, c("variable_id", "variable_id")] %>% 
  ...
  dplyr::mutate(class = case_when(
    ...
    !is.na(true_name) ~ "nasal_microbiome"
  )) %>%
...
writeDataTable(wb, sheet = 2, x = edge_data,
               colNames = TRUE, rowNames = FALSE)
saveWorkbook(wb = wb, file = "stool_microbiome_vs_nasal_microbiome.xlsx", overwrite = TRUE)
...
plot <-
  ...
  #                                 # label = ifelse(class == "nasal_microbiome", NA, true_name),
  #                                 label = true_name,
  ...
  scale_color_manual(values = omics_color[c("nasal_microbiome", "Stool microbiome")]) +
...
ggsave(plot,
       filename = "stool_microbiome_nasal_microbiome_cor_plot_example.pdf",
       width = 8.5,
       height = 7)
