...
oral_nasal_sample_wise_oral_dim = 
  dim(oral_microbiome_expression_data)
...
sum(oral_microbiome_nasal_microbiome_lm_adjusted_cor_spearman$p_adjust < 0.2)
...
writeDataTable(wb, sheet = 2, x = edge_data,
               colNames = TRUE, rowNames = FALSE)
saveWorkbook(wb = wb, file = "oral_microbiome_vs_nasal_microbiome.xlsx", overwrite = TRUE)
...
plot <-
  ...
  scale_color_manual(values = omics_color[c("nasal_microbiome", "oral microbiome")]) +
...
ggsave(plot,
       filename = "oral_microbiome_nasal_microbiome_cor_plot_example.pdf",
       width = 8.5,
       height = 7)
