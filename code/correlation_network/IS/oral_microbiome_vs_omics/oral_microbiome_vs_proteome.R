...
######work directory
setwd(masstools::get_project_wd())
setwd("data_analysis/oral_microbiome_vs_proteome_IS")
...
###plasma proteome
{
  load(here::here("data_analysis/proteome/data_preparation/expression_data"))
  load(here::here("data_analysis/proteome/data_preparation/sample_info"))
  load(here::here("data_analysis/proteome/data_preparation/variable_info"))  
}
...
###remain samples with IS
proteome_sample_info =
  proteome_sample_info %>%
  dplyr::left_join(oral_microbiome_sample_info[,c("sample_id", "IRIS")],
                   by = "sample_id") %>% 
  dplyr::filter(IRIS == "IS")
...
which(proteome_sample_info$subject_id != 
oral_microbiome_sample_info$subject_id)

proteome_sample_info[91,]$subject_id

proteome_sample_info[91,]
oral_microbiome_sample_info[91,]
...
oral_microbiome_sample_info$subject_id == proteome_sample_info$subject_id
...
# oral_microbiome_proteome_lm_adjusted_cor =
#   lm_adjusted_cor(
#     data_set1 = oral_microbiome_expression_data,
#     data_set2 = proteome_expression_data,
#     sample_info = oral_microbiome_sample_info,
#     method = "all",
#     threads = 8
#   )
# 
# oral_microbiome_proteome_lm_adjusted_cor_spearman = oral_microbiome_proteome_lm_adjusted_cor[[1]]
# 
# save(
#   oral_microbiome_proteome_lm_adjusted_cor_spearman,
#   file = "oral_microbiome_proteome_lm_adjusted_cor_spearman",
#   compress = "xz"
# )

load("oral_microbiome_proteome_lm_adjusted_cor_spearman")
###here we use the lm_adjusted_cor
sum(oral_microbiome_proteome_lm_adjusted_cor_spearman$p_adjust < 0.2)
...
node_data =
  ...
  dplyr::left_join(proteome_variable_info[, c("variable_id", "variable_id")] %>% 
...
writeDataTable(wb, sheet = 2, x = edge_data,
               colNames = TRUE, rowNames = FALSE)
saveWorkbook(wb = wb, file = "oral_microbiome_vs_proteome.xlsx", overwrite = TRUE)
...
plot <-
  ...
  #                                 # label = ifelse(class == "Proteome", NA, true_name),
  #                                 label = true_name,
  ...
  scale_color_manual(values = omics_color[c("Proteome", "Oral microbiome")]) +
...
# ggsave(plot,
#        filename = "oral_microbiome_proteome_cor_plot_example.pdf",
#        width = 8.5,
#        height = 7)
