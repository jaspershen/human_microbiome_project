#' ---
#' title: "Oral microbiome cytokine correlation"
#' author: 
#'   - name: "Xiaotao Shen" 
...
######work directory
setwd(masstools::get_project_wd())
dir.create("data_analysis/correlation_network/whole_data_set_IS/oral_microbiome_vs_cytokine")
setwd("data_analysis/correlation_network/whole_data_set_IS/oral_microbiome_vs_cytokine")
...
###plasma cytokine
{
  load(here::here("data_analysis/cytokine/data_preparation/expression_data"))
...
which(cytokine_sample_info$subject_id !=
oral_microbiome_sample_info$subject_id)

cytokine_sample_info[91,]$subject_id

cytokine_sample_info[91,]
...
oral_microbiome_sample_info$subject_id == cytokine_sample_info$subject_id
...
oral_microbiome_cytokine_lm_adjusted_cor =
  lm_adjusted_cor(
...
oral_microbiome_cytokine_lm_adjusted_cor_spearman = oral_microbiome_cytokine_lm_adjusted_cor[[1]]

save(
  oral_microbiome_cytokine_lm_adjusted_cor_spearman,
  file = "oral_microbiome_cytokine_lm_adjusted_cor_spearman",
  compress = "xz"
)

load("oral_microbiome_cytokine_lm_adjusted_cor_spearman")
###here we use the lm_adjusted_cor
sum(oral_microbiome_cytokine_lm_adjusted_cor_spearman$p_adjust < 0.05)
...
writeDataTable(wb, sheet = 2, x = edge_data,
               colNames = TRUE, rowNames = FALSE)
saveWorkbook(wb = wb, file = "oral_microbiome_vs_cytokine.xlsx", overwrite = TRUE)
...
plot <-
  ...
  scale_color_manual(values = omics_color[c("Cytokine", "Oral microbiome")]) +
...
extrafont::loadfonts()
ggsave(plot,
       filename = "oral_microbiome_cytokine_cor_plot_example.pdf",
       width = 14,
       height = 12)
