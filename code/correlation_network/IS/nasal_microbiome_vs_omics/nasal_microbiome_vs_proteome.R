...
######work directory
setwd(masstools::get_project_wd())
setwd("data_analysis/correlation_network/IS/nasal_microbiome_vs_proteome_IS")
...
#####only remain the samples with IS information
...
###only remain samples with IS
proteome_sample_info = 
  proteome_sample_info %>% 
  dplyr::left_join(nasal_microbiome_sample_info[,c("sample_id", "IRIS")], 
                   by = "sample_id") %>% 
  dplyr::filter(IRIS == "IS")
...
nasal_microbiome_sample_info$subject_id == proteome_sample_info$subject_id
...
# nasal_microbiome_proteome_lm_adjusted_cor =
#   lm_adjusted_cor(
...
#   )
# 
# nasal_microbiome_proteome_lm_adjusted_cor_spearman = nasal_microbiome_proteome_lm_adjusted_cor[[1]]
# 
# save(
#   nasal_microbiome_proteome_lm_adjusted_cor_spearman,
#   file = "nasal_microbiome_proteome_lm_adjusted_cor_spearman",
#   compress = "xz"
# )

load("nasal_microbiome_proteome_lm_adjusted_cor_spearman")
###here we use the lm_adjusted_cor
sum(nasal_microbiome_proteome_lm_adjusted_cor_spearman$p_adjust < 0.2)
...
writeDataTable(wb, sheet = 2, x = edge_data,
               colNames = TRUE, rowNames = FALSE)
saveWorkbook(wb = wb, file = "nasal_microbiome_vs_proteome.xlsx", overwrite = TRUE)
...
plot <-
  ...
  scale_color_manual(values = omics_color[c("Proteome", "Nasal microbiome")]) +
...
ggsave(plot,
       filename = "nasal_microbiome_proteome_cor_plot_example.pdf",
       width = 8.5,
       height = 7)
