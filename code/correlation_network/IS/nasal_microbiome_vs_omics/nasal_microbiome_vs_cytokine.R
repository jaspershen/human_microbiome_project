#' ---
#' title: "Nasal microbiome cytokine correlation"
#' author: 
#'   - name: "Xiaotao Shen" 
...
######work directory
setwd(masstools::get_project_wd())
dir.create("data_analysis/correlation_network/whole_data_set_IS/nasal_microbiome_vs_cytokine", recursive = TRUE)
setwd("data_analysis/correlation_network/whole_data_set_IS/nasal_microbiome_vs_cytokine")
...
nasal_microbiome_sample_info$subject_id == cytokine_sample_info$subject_id
...
#
nasal_microbiome_cytokine_lm_adjusted_cor =
  lm_adjusted_cor(
...
nasal_microbiome_cytokine_lm_adjusted_cor_spearman = nasal_microbiome_cytokine_lm_adjusted_cor[[1]]

save(
  nasal_microbiome_cytokine_lm_adjusted_cor_spearman,
  file = "nasal_microbiome_cytokine_lm_adjusted_cor_spearman",
  compress = "xz"
)

load("nasal_microbiome_cytokine_lm_adjusted_cor_spearman")
###here we use the lm_adjusted_cor
sum(nasal_microbiome_cytokine_lm_adjusted_cor_spearman$p_adjust < 0.2)
...
writeDataTable(wb, sheet = 2, x = edge_data,
               colNames = TRUE, rowNames = FALSE)
saveWorkbook(wb = wb, file = "nasal_microbiome_vs_cytokine.xlsx", overwrite = TRUE)
...
plot <-
  ...
  scale_color_manual(values = omics_color[c("Cytokine", "Nasal microbiome")]) +
...
ggsave(plot,
       filename = "nasal_microbiome_cytokine_cor_plot_example.pdf",
       width = 14,
       height = 12)
