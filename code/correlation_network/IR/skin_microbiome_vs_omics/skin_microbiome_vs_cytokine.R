#' ---
#' title: "Skin microbiome cytokine correlation"
#' author: 
#'   - name: "Xiaotao Shen" 
...
######work directory
setwd(masstools::get_project_wd())
dir.create("data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_cytokine")
setwd("data_analysis/correlation_network/whole_data_set_IR/skin_microbiome_vs_cytokine")
...
skin_microbiome_sample_info$subject_id == cytokine_sample_info$subject_id
...
skin_microbiome_cytokine_lm_adjusted_cor =
  lm_adjusted_cor(
...
skin_microbiome_cytokine_lm_adjusted_cor_spearman = skin_microbiome_cytokine_lm_adjusted_cor[[1]]

save(
  skin_microbiome_cytokine_lm_adjusted_cor_spearman,
  file = "skin_microbiome_cytokine_lm_adjusted_cor_spearman",
  compress = "xz"
)

load("skin_microbiome_cytokine_lm_adjusted_cor_spearman")


###here we use the lm_adjusted_cor
sum(skin_microbiome_cytokine_lm_adjusted_cor_spearman$p_adjust < 0.2)
...
writeDataTable(wb, sheet = 2, x = edge_data,
               colNames = TRUE, rowNames = FALSE)
saveWorkbook(wb = wb, file = "skin_microbiome_vs_cytokine.xlsx", overwrite = TRUE)
...
plot <-
  ...
  scale_color_manual(values = omics_color[c("Cytokine", "Skin microbiome")]) +
...
extrafont::loadfonts()
ggsave(plot,
       filename = "skin_microbiome_cytokine_cor_plot_example.pdf",
       width = 14,
       height = 12)
