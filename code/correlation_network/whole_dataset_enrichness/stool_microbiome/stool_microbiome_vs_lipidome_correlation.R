...
setwd("data_analysis/correlation_network/whole_data_set_enrichness/stool_microbiome_vs_lipidome")

stool_microbiome_expression_data =
  t(richness_ST_byGenus) %>%
  as.data.frame()

colnames(stool_microbiome_expression_data) = 
  sample_info$sample_id[match(colnames(stool_microbiome_expression_data), sample_info$RandomID)]

stool_microbiome_sample_info = 
  sample_info[match(colnames(stool_microbiome_expression_data), sample_info$sample_id),]

colnames(stool_microbiome_expression_data) == stool_microbiome_sample_info$sample_id

stool_microbiome_variable_info = 
  data.frame(variable_id = rownames(stool_microbiome_expression_data),
             Genus = rownames(stool_microbiome_expression_data))
...
sum(lipidome_sample_info$sample_id == stool_microbiome_sample_info$sample_id)
sum(lipidome_sample_info$subject_id == stool_microbiome_sample_info$subject_id)
...
#   )
# 
# stool_microbiome_lipidome_lm_adjusted_cor_spearman = stool_microbiome_lipidome_lm_adjusted_cor[[1]]
# 
# save(
#   stool_microbiome_lipidome_lm_adjusted_cor_spearman,
#   file = "stool_microbiome_lipidome_lm_adjusted_cor_spearman",
#   compress = "xz"
# )

load("stool_microbiome_lipidome_lm_adjusted_cor_spearman")

###here we use the lm_adjusted_cor
sum(stool_microbiome_lipidome_lm_adjusted_cor_spearman$p_adjust < 0.2)
sum(stool_microbiome_lipidome_lm_adjusted_cor_spearman$p_adjust2 < 0.2)
...
plot <-
  ...
  scale_color_manual(values = omics_color[c("Lipidome", "Stool microbiome")]) +
