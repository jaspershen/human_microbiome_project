...
c3 %>%
  ggplot(aes(x = genus, y = estimate, ymin = conf.low, ymax = conf.high)) +
  geom_pointrange() +
  coord_flip() +
  theme_bw() +
...
genus.vec <- 
  ...
  dplyr::select(!!sym(term)) %>%
  dplyr::mutate(genus = !!m$formula$resp) %>%
  dplyr::rename(value = !!sym(term)) %>%
  dplyr::mutate(cytokine = tsub)
