...
# x = dir() %>%
#   grep("brm_oral_", ., value = TRUE)
# 
# result = vector(mode = "list", length = length(x))
# names(result) =
#   x %>%
#   stringr::str_replace("brm_oral_", "") %>%
...
##/Users/xiaotaoshen/Box/Xiaotao Shen's Files/human_microbiome_project/data_analysis/brms/oral/result

####next code are in local PC
setwd(masstools::get_project_wd())
setwd("data_analysis/brms/oral/result/")
rm(list = ls())
...
####output result
library(openxlsx)
openxlsx::write.xlsx(final_table, "oral_cytokine_result.xlsx", asTable = TRUE, overwrite = TRUE)
...
ggsave(plot,
       filename = "brms_oral_microbiome_vs_cytokine.pdf",
       width = 14,
       height = 7)
...
result <- 
  data.frame(result, class = "Oral")

openxlsx::write.xlsx(
  result,
  "oral_cytokine_beta.xlsx",
  asTable = TRUE,
  overwrite = TRUE
)
